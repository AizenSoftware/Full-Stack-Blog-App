...
const CommentsContainer = ({
  ...
  const { mutate: mutateNewComment, isLoading: isLoadingNewComment } =
    useMutation({
      ...
      onSuccess: () => {
        toast.success("Your comment is successfully send it");
  ...
  const { mutate: mutateCheckComment } = useMutation({
    mutationFn: ({ id }) => {
      return checkCommentById({ id });
    },
    onSuccess: () => {
      toast.success("commen is working");
      queryClient.invalidateQueries(["blog", postSlug]);
    },
    onError: (error) => {
      toast.error(error.message);
  ...
  const checkCommentById = (value, commentId) => {
    checkCommentById({
      desc: value,
      commentId,
    });
